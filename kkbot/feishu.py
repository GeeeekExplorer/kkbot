    ...
    def _parse(lc: dict) -> tuple[str, list[str]]:
        ...
        for block in lc.get("content", []):
            for el in (block if isinstance(block, list) else []):
                tag = el.get("tag")
                if tag in ("text", "a"):   texts.append(el.get("text", ""))
                elif tag == "at":          texts.append(f"@{el.get('user_name', 'user')}")
                elif tag == "img":
                    if k := el.get("image_key"): imgs.append(k)
...
class FeishuBot:
    ...
    async def start(self) -> None:
        if not self.app_id or not self.app_secret:
            logger.error("Feishu credentials not configured"); return
        self._running = True
        ...
        def _ws_thread():
            while self._running:
                try:   ws.start()
                except Exception as e: logger.warning("WS error: {}", e)
                if self._running:
    ...
    async def _handle(self, data: Any) -> None:
        try:
            ...
            mid = msg.message_id
            if mid in self._dedup: return
            self._dedup[mid] = None
            while len(self._dedup) > 1000: self._dedup.popitem(last=False)

            if sender.sender_type == "bot": return

            if msg.chat_type == "group":
                mentions = msg.mentions or []
                if not any(getattr(getattr(m, "id", None), "open_id", None) == SELF_OPEN_ID
                           for m in mentions):
                    return
            ...
            if mtype == "text":
                ...
                if t: text_parts.append(_AT_RE.sub("", t).strip())
                for k in img_keys:
                    if b64 := await self._img_b64(mid, k): images_b64.append(b64)
            elif mtype == "image":
            ...
            if not text and not images_b64: return
    ...
    async def send(self, chat_id: str, text: str) -> None:
        if not self._client:
            logger.warning("Client not ready"); return
        ...
        if _MENTION_RE.search(text):
            ...
            await loop.run_in_executor(None, self._send_raw, id_type, chat_id, "interactive", content)
        else:
    ...
    def _build_post(self, text: str) -> dict:
        ...
        for i, part in enumerate(parts):
            if i % 2 == 0:
                if part.strip(): content.append({"tag": "text", "text": part})
            else:               content.append({"tag": "at", "user_id": part})
    ...
    def _send_raw(self, id_type: str, receive_id: str, msg_type: str, content: str) -> bool:
        try:
            ...
            if not resp.success():
                logger.error("Send failed: {} {}", resp.code, resp.msg); return False
            return True
        except Exception as e:
            logger.error("Send error: {}", e); return False
            ...
            with open(path, "rb") as f:
                req  = (CreateImageRequest.builder()
                        .request_body(CreateImageRequestBody.builder()
                                      .image_type("message").image(f).build()).build())
